...
# ==================== 配置 ====================
API_KEY = os.getenv("BINANCE_API_KEY", "your_api_key_here")  # 从环境变量获取
API_SECRET = os.getenv("BINANCE_API_SECRET", "your_api_secret_here")  # 从环境变量获取
COIN_NAME = os.getenv("COIN_NAME", "DOGE")  # 交易币种
CONTRACT_TYPE = os.getenv("CONTRACT_TYPE", "USDT")  # 合约类型：USDT 或 USDC
GRID_SPACING = float(os.getenv("GRID_SPACING", "0.001"))  # 网格间距 (0.1%)
INITIAL_QUANTITY = int(os.getenv("INITIAL_QUANTITY", "10"))  # 初始交易数量 (币数量)
LEVERAGE = int(os.getenv("LEVERAGE", "20"))  # 杠杆倍数
WEBSOCKET_URL = "wss://fstream.binance.com/ws"  # WebSocket URL
POSITION_THRESHOLD = int(os.getenv("POSITION_THRESHOLD", "500"))  # 锁仓阈值
POSITION_LIMIT = int(os.getenv("POSITION_LIMIT", "100"))  # 持仓数量阈值
SYNC_TIME = int(os.getenv("SYNC_TIME", "10"))  # 同步时间（秒）
ORDER_FIRST_TIME = int(os.getenv("ORDER_FIRST_TIME", "10"))  # 首单间隔时间
