...
class ShortAccountExecutor(HedgeGridExecutor):
    ...
    async def _place_open_order(self, level: GridLevel) -> Optional[TrackedOrder]:
        ...
        try:
            ...
            if self.exchange:
                ...
                # 模拟订单ID
                order_id = f"SHORT_SELL_{level.level_id}_{datetime.utcnow().timestamp()}"
            ...
            self.logger.info(f"空头开仓订单已下: {order_id}, 价格: {order_candidate.price}, 数量: {order_candidate.amount}")
            return tracked_order
            
        except Exception as e:
            self.logger.error(f"空头开仓订单失败: {e}")
            return None
    ...
    async def _place_close_order(self, level: GridLevel) -> Optional[TrackedOrder]:
        ...
        try:
            ...
            # 创建买入订单候选
            order_candidate = self._create_buy_order_candidate(level, take_profit_price, level.amount)
            ...
            if self.exchange:
                ...
                # 模拟订单ID
                order_id = f"SHORT_BUY_{level.level_id}_{datetime.utcnow().timestamp()}"
            ...
            self.logger.info(f"空头平仓订单已下: {order_id}, 价格: {order_candidate.price}, 数量: {order_candidate.amount}")
            return tracked_order
            
        except Exception as e:
            self.logger.error(f"空头平仓订单失败: {e}")
            return None
